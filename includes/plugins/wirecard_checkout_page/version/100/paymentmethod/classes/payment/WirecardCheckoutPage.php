...
class WirecardCheckoutPage extends PaymentMethod
{
        ...
        try {
            ...
            switch ($return->getPaymentState()) {
                case WirecardCEE_QPay_ReturnFactory::STATE_SUCCESS:
                    ...
                    $incomingPayment->cHinweis = sprintf(
                        'OrderNumber %s GatewayReferenceNumber %s',
    ...
    protected function setConsumerInformation($order, WirecardCEE_Stdlib_ConsumerData $consumerData)
    {
        ...
        if (strlen($customer->dGeburtstag) && $customer->dGeburtstag != '00.00.0000') {
            $consumerData->setBirthDate(new DateTime($customer->dGeburtstag));
        }
        ...
        $billingAddress->setFirstname($order->oRechnungsadresse->cVorname)
            ->setLastname($order->oRechnungsadresse->cNachname)
            ->setAddress1($order->oRechnungsadresse->cStrasse . ' ' . $order->oRechnungsadresse->cHausnummer)
            ->setCity($order->oRechnungsadresse->cOrt)
        ...
        $shippingAddress->setFirstname($src->cVorname)
            ->setLastname($src->cNachname)
            ->setAddress1($src->cStrasse . ' ' . $order->Lieferadresse->cHausnummer)
            ->setCity($src->cOrt)
            ->setZipCode($src->cPLZ)
            ->setCountry($countryCode)
            ->setPhone($src->cTel);
